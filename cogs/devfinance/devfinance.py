...
class DevFinanceAPI:
    """API DevFinance | Système commun de monnaie sur le bot (Branche de développement)"""
    # But = Unifier l'API utilisateur et les paramètres de chaque serveur en une seule API sans tout casser
    ...
    def backup(self):
        ...
        fileIO("data/devfinance/eco.json", "save", backup)
        return True

    def _save(self):
        """Sauvegarde l'API seulement"""
        fileIO("data/devfinance/eco.json", "save", self.eco)
        return True

    def new_user(self, user: discord.Member):
        """Création d'un nouveau profil

        Renvoie True si créé, False si déjà présent"""
        server = user.server
        if server.id not in self.eco["USERS"]:
            self.eco["USERS"][server.id] = {}
        if user.id not in self.eco["USERS"]:
            self.eco["USERS"][server.id][user.id] = {"SOLDE": 100,
                                            "TRSAC": [],
                                            "EXTRA": {},
    ...
    def get_user(self, user: discord.Member, w: bool = False, m: bool = False):
        ...
        if server.id not in self.eco["USERS"]:
            self.eco["USERS"][server.id] = {}
            self._save()
        if user.id not in self.eco["USERS"][server.id]:
            if m:  # make
                self.new_user(user)
            else:
                return False
        if not w:  # write
            user = self.eco["USERS"][server.id][user.id]
            ...
            return self.eco["USERS"][server.id][user.id]
    ...
    def obj_transaction(self, trans: list):
        ...
        servid = userid = None
        for s in self.eco["USERS"]:
            for u in self.eco["USERS"][s]:
                if trans in self.eco["USERS"][s][u]["TRSAC"]:
                    servid = s
                    userid = u
    ...
    def id_to_transaction_obj(self, transaction_id: str, w: bool=False):
        ...
        Renvoie obj_transaction()"""
        for serv in self.eco["USERS"]:
            for user in self.eco["USERS"][serv]:
                for tr in self.eco["USERS"][serv][user]["TRSAC"]:
    ...
    def total_server_credits(self, server: discord.Server):
        ...
        if server.id not in self.eco["USERS"]:
            ...
            self._save()
        total = 0
        for u in self.eco["USERS"][server.id]:
            total += self.eco["USERS"][server.id][u]["SOLDE"]
        return total
    ...
    def transfert_credits(self, crean: discord.Member, debit: discord.Member, nombre: int, raison: str):
        ...
        if crean is debit:
            return False
        debitdata = self.get_user(debit, True)
        if nombre > 0:
        ...
        if not server:
            for serv in self.eco["USERS"]:
                ...
                for member in server.members:
                    liste.append(self.get_user(member))
        else:
            for member in server.members:
                liste.append(self.get_user(member))
        if liste:
    ...
    def gen_palmares(self, server: discord.Server, nombre: int):
        ...
        if server.id in self.eco["USERS"]:
            liste = [[self.eco["USERS"][server.id][u]["SOLDE"], u] for u in self.eco["USERS"][server.id]]
    ...
    def reset_user_data(self, user: discord.Member):
        ...
        server = user.server
        if server.id in self.eco["USERS"]:
            if user.id in self.eco["USERS"][server.id]:
                del self.eco["USERS"][server.id][user.id]
                self._save()
                return True
        return False
...
class DevFinance:
    ...
    def __init__(self, bot):
        self.bot = bot
        self.api = DevFinanceAPI(bot, "data/devfinance/eco.json")
        self.sys = self.api.get_system()
        self.sys_defaut = {"MONEY_NAME": "crédit", "MONEY_NAME_PLURIEL": "crédits", "MONEY_SYMBOLE": "cds",
    ...
    async def new(self, ctx):
        ...
        if rep is None or rep.reaction.emoji == "✖":
            ...
            done = self.api.new_user(ctx.message.author)
            if done:
    ...
    async def compte(self, ctx, user: discord.Member = None):
        ...
        data = self.api.get_user(user) if user else self.api.get_user(ctx.message.author)
    ...
    async def unlock(self, ctx, code: str):
        ...
        data = self.api.get_user(ctx.message.author)
        if not data:
    ...
    async def give(self, ctx, user: discord.Member, somme: int, *raison):
        ...
        if self.api.get_user(user):
            if self.api.get_user(ctx.message.author):
    ...
    async def revenu(self, ctx):
        ...
        data = self.api.get_user(user, True)
        if not self.api.get_user(user):
            await self.bot.say("**Vous n'avez pas de compte** ─ Ouvrez-en un avec `{}b new`".format(ctx.prefix))
            return
        ...
        if self.api.get_user(user).solde < 10000:
...
# ------------- MODERATION ---------------------
    ...
    @_modbanque.command(pass_context=True)
    async def backupbank(self, ctx):
        """Backup toutes les données au dernier enregistrement"""
        self.api.backup()
        await self.bot.say("Effectué avec succès")

def check_folders():
    if not os.path.exists("data/devfinance"):
        print("Creation du fichier Finance ...")
        os.makedirs("data/devfinance")


def check_files():
    if not os.path.isfile("data/devfinance/eco.json"):
        print("Création de devfinance/eco.json ...")
        fileIO("data/devfinance/eco.json", "save", {"USERS": {}, "SYSTEM": {}})
